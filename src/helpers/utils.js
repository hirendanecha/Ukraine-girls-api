...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.userName;
  let msg = `You have been assign in UkrainianGirls channel by the UkrainianGirls Admin.
  To access your channel, log into your UkrainianGirls account,click on the
  UkrainianGirls icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `UkrainianGirls has approved your Connection application.`;
    } else {
      msg = `UkrainianGirls has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
